...
import Form from "@/Pages/Settings/KonfigurasiPajak/Form";

export default function Index({ konfigurasiPajak, filters, flashMessage }) {
    const endpoint = "/settings/konfigurasi-pajak";
    ...
    const removeItem = (id) => {
        if (!window.confirm("Hapus konfigurasi pajak ini?")) return;
        router.post(`${endpoint}/delete`, { id: String(id) }, { preserveScroll: true });
    };

    return (
        <DashboardLayout title="Konfigurasi Pajak">
            <Head title="Konfigurasi Pajak" />
            ...
            <div className="space-y-6">
                <section className="rounded-3xl border bg-white p-6 shadow-sm md:p-8">
                    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            ...
                            <h1 className="text-2xl font-semibold text-slate-900 md:text-3xl">Konfigurasi Pajak</h1>
                            <p className="mt-1 text-sm text-slate-600">Atur parameter pajak untuk perhitungan transaksi.</p>
                        </div>
                        ...
                        <Dialog open={openCreate} onOpenChange={setOpenCreate}>
                            ...
                            <DialogContent className="sm:max-w-xl">
                                <DialogHeader>
                                    <DialogTitle>Tambah Konfigurasi Pajak</DialogTitle>
                                    <DialogDescription>Isi parameter pajak sesuai kebijakan toko.</DialogDescription>
                                </DialogHeader>
                ...
                <section className="rounded-3xl border bg-white p-4 shadow-sm md:p-6">
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Dialog open={editingItem?.id === item.id} onOpenChange={(open) => setEditingItem(open ? item : null)}>
                                                        ...
                                                        <DialogContent className="sm:max-w-xl">
                                                            <DialogHeader>
                                                                <DialogTitle>Edit Konfigurasi Pajak</DialogTitle>
                                                                <DialogDescription>Perbarui konfigurasi pajak.</DialogDescription>
                                                            </DialogHeader>
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
                        <span>Halaman {konfigurasiPajak.meta.current_page} dari {konfigurasiPajak.meta.last_page} | Total {konfigurasiPajak.meta.total} data</span>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" disabled={konfigurasiPajak.meta.current_page <= 1} onClick={() => goPage(konfigurasiPajak.meta.current_page - 1)}>Sebelumnya</Button>
                            <Button variant="outline" size="sm" disabled={konfigurasiPajak.meta.current_page >= konfigurasiPajak.meta.last_page} onClick={() => goPage(konfigurasiPajak.meta.current_page + 1)}>Berikutnya</Button>
                        </div>
                    </div>
                </section>
            </div>
