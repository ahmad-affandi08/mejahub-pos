...
export default function Index({ memberships, filters, flashMessage }) {
    const endpoint = "/crm/membership";
    const searchValue = filters?.search ?? "";
    const rows = memberships?.data ?? [];
    const hasData = rows.length > 0;
    const currentPage = memberships?.meta?.current_page ?? 1;
    const lastPage = memberships?.meta?.last_page ?? 1;
    const total = memberships?.meta?.total ?? 0;
        ...
        <DashboardLayout title="Membership">
            ...
            <div className="space-y-6">
                <section className="rounded-3xl border bg-white p-6 shadow-sm md:p-8">
                    ...
                    <h1 className="text-2xl font-semibold text-slate-900 md:text-3xl">Membership</h1>
                    <p className="mt-2 text-sm text-slate-600">Segmentasi tier pelanggan berdasarkan total belanja historis.</p>
                </section>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Rata-Rata Transaksi</TableHead>
                            ...
                            {hasData ? (
                                rows.map((item, index) => (
                                    <TableRow key={`${item.nama_pelanggan}-${index}`}>
                                        ...
                                        <TableCell>{formatIDR(item.rata_rata_transaksi)}</TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="py-12 text-center text-sm text-muted-foreground">
                                        Belum ada data membership dari transaksi POS.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
